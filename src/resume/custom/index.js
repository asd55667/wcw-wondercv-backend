...
const router = new Router({ prefix: "/custom" });
...
router.get("/:id", cache(300), async (ctx) => {
  const result = await Custom.findById(ctx.params.id);
  if (!result) {
    ctx.throw(404);
  }
  ctx.status = 200;
...
router.post("/query", cache(300), async (ctx) => {
  ...
  try {
    const result = await Custom.paginate(query, options);
    ctx.status = 200;
    ctx.body = result;
  } catch (error) {
    ctx.throw(400, error.message);
...
router.post("/", auth, authz("crew:create"), async (ctx) => {
  try {
    const crew = new Custom(ctx.request.body);
    await crew.save();
    ctx.status = 201;
  } catch (error) {
    ctx.throw(400, error.message);
...
router.patch("/:id", auth, authz("crew:update"), async (ctx) => {
  try {
    await Custom.findByIdAndUpdate(ctx.params.id, ctx.request.body, {
      runValidators: true,
    });
    ctx.status = 200;
  } catch (error) {
...
router.delete("/:id", auth, authz("crew:delete"), async (ctx) => {
  try {
    await Custom.findByIdAndDelete(ctx.params.id);
    ctx.status = 200;
  } catch (error) {
    ctx.throw(400, error.message);
  }
